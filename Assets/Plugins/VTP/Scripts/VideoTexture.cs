...
//----(DO NOT USE- NOT FOR YOU TO TOUCH!, USE PUBLIC GETTERS)---------
	...
	[DllImport ("VTPAVF")]
	private static extern bool update(IntPtr videoTextureInstance);	
...
//		returnedToBeginning = true;
	...
	public VideoLoopType LoopType{
		...
		set { 
			loopType = value; 
			if(videoInstance != (IntPtr)0){
				if(isDebug) Debug.Log ("setting loop state! : " + LoopType);
	...
	public float VideoSpeed{
		...
		set {
			videoSpeed = value;
			if(videoInstance != (IntPtr)0){
				if(isDebug) Debug.Log ("setting speed to " + videoSpeed);
	...
	public bool IsPaused{
		...
		set { 			
			isPaused = value;
			if(videoInstance != (IntPtr)0){
				if(isDebug) Debug.Log ("PAUSING! : " +  isPaused);
	...
	public float VideoTime{
		...
		set { 			
			videoTime = value;
			if(videoInstance != (IntPtr)0){
//				if(isDebug) Debug.Log ("setting the time to : " + videoTime);
	...
	public float VideoVolume {
		...
		set{
			videoVolume	= value;
			if(videoInstance != (IntPtr)0){
				if(isDebug) Debug.Log ("setting the volume to : " + videoVolume);
...
//			if(isDebug) Debug.Log ("PLAY_QUEUE_AND_STOP complete. Press play to continue: " + videoPaths[currentlyPlayingIndex]);
			//force play mode if paused
			isPaused = true;
			videoTime = 0;
		}
		else if(loopType == VideoLoopType.PLAY_VIDEO_AND_STOP){
//			if(isDebug) Debug.Log ("PLAY_VIDEO_AND_STOP complete. Press play to continue: " + videoPaths[currentlyPlayingIndex]);
			//force play mode if paused
			isPaused = true;
			videoTime = 0;
		}	
		else if(loopType == VideoLoopType.LOOP_VIDEO || loopType == VideoLoopType.LOOP_QUEUE && videoPaths.Length == 1){
//			if(isDebug) Debug.Log ( " LOOP_VIDEO complete. Looping: " + videoPaths[currentlyPlayingIndex]);
		}
	...
	public void jumpToVideo(int i ){
		//force play mode if paused
		isPaused = false;
		if(isDebug) Debug.Log ("jumping to " + i);
	...
	public void CopyVideoPixels(){
		if(IsVideoLoaded){
			...
			if(ptrResults != IntPtr.Zero && dataSize > 0){
				...
				//				if(isDebug) Debug.Log (videoData.data[0] + " " + videoData.data[1] + " " + videoData.data[2]);
			}
		}
	}
	...
	public void videoLoaded(bool loadSuccess, int idx){
		...
		if(currentlyPlayingIndex >= videoPaths.Length){
			...
			if(isDebug) Debug.Log ("clamping currentlyPlayingIndex- you changed the # of videos without reloading them.");
		}
		
		if(loadSuccess)
		{
//			if(isDebug) Debug.Log ("LOADED + " + videoPaths[currentlyPlayingIndex] + " : " + loadSuccess);
...
//----------------------------------------------
	...
	public void UpdateTexture(int w, int h, IntPtr texID, int w2, int h2, IntPtr texID2){
		...
		if(destroyed || gameObject == null) //maybe we're destroying this?
		{
			return;
		}

		if(backingType == VideoTextureBackingType.PIXELS_ONLY || backingType == VideoTextureBackingType.TEXTURE_AND_PIXELS || backingType == VideoTextureBackingType.TEXTURE_ONLY){
...
//		refreshTextureValues();
	...
	private void initVideoProperties(){
		if(isDebug) Debug.Log ("INITTING VOLUME TO " + this.videoVolume);
...
//		GL.InvalidateState();
	...
	/// <summary>
	/// about to be destroyed- only perform destroy activities if necessary.
	/// </summary>
